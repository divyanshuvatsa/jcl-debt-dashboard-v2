...
class Dashboard:
    ...
    def render_sidebar(self) -> dict:
        st.sidebar.markdown(
            f'<div class="terminal-header">▌ JCL TERMINAL</div>',
            unsafe_allow_html=True
        )
        st.sidebar.caption("Bloomberg-style debt monitor v2.0")

        # Data source indicator
        src = self.raw.get("_source", "unknown")
        if src == "excel":
            st.sidebar.success(f"📊 Data source: Excel (₹{self.raw.get('_total_check', 0):.1f} Cr)")
        else:
            st.sidebar.info(f"📊 Data source: Hardcoded fallback")

        st.sidebar.divider()

        # === Excel upload ===
        with st.sidebar.expander("📁 Upload Excel Model", expanded=False):
            uploaded = st.file_uploader(
                "Replace with new Excel",
                type=["xlsx"],
                help="Re-upload JCL_Debt_Model.xlsx to refresh data"
            )
            if uploaded:
                from data.excel_loader import load_from_excel
                new = load_from_excel(uploaded)
                if new:
                    st.session_state["uploaded_data"] = new
                    st.success("Loaded! Refresh page to apply.")
                else:
                    st.error("Could not parse Excel.")

        # === Snapshot date ===
        as_of = st.sidebar.date_input(
            "📅 As-of Date",
            value=date(2026, 4, 27),
            help="Snapshot date for maturity & alert calculations"
        )

        # === Live FX rate ===
        fx_data = live_data.get_fx()
        st.sidebar.markdown(
            f"💱 **USD/INR**: {fx_data['rate']:.2f} "
            f"<span style='color:{COLORS['text_dim']}; font-size:0.75rem;'>({fx_data['source']})</span>",
            unsafe_allow_html=True
        )
        fx_override = st.sidebar.number_input(
            "Override FX rate", value=float(fx_data["rate"]),
        ...
        basis = st.sidebar.radio(
            "📊 Financial Basis",
            options=["FY26E", "FY24A"],
            help="FY26E = projected (recommended) | FY24A = audited"
        )
        ...
        if api_key and ai_assistant.anthropic is not None:
            ...
            st.sidebar.warning("🤖 AI: Add ANTHROPIC_API_KEY in .streamlit/secrets.toml")

        st.sidebar.divider()

        # === Help ===
        with st.sidebar.expander("ℹ️ How to read this", expanded=False):
            st.markdown("""
**Color codes:**
- 🟢 Green = Compliant / Healthy
- 🟡 Amber = Near breach (within 10%)
- 🔴 Red = Breached / Action needed

**Tabs:**
- **Overview** — start here
- **Liquidity** — debt maturity & repayment
- **Covenants** — compliance status
- **Scenarios** — stress testing
- **Ask AI** — natural language Q&A
- **Intelligence** — portfolio analytics
- **Export** — Board memo, emails

All ₹ in Crores. Ratios as multiples (e.g. 1.39x).
            """)
